---
name: today
description: docs/TASKS.md를 읽고 오늘 할 일을 정리해주는 skill. 지금 할 것 / 다음에 할 것 / 완료 섹션을 구분 요약하고, 바로 시작 가능한 작업 1~3개와 각 작업별 읽어야 할 파일 목록을 안내한다.
---
...
- `docs/TASKS.md` — 현재 작업 큐 확인 (이미 읽음)
```

파일 목록은 작업 성격에 맞게 실제로 존재할 가능성이 높은 파일만 안내한다.
불필요한 파일은 포함하지 않는다.

## 5. 세션 종료 시 docs/TASKS.md 업데이트 권장

브리핑 마지막에 아래 문구를 반드시 포함한다:

---
> **세션 종료 전 `docs/TASKS.md`를 업데이트해야 한다.**
> - 완료한 항목은 `## 완료` 섹션으로 이동
> - 진행 중인 항목은 `## 진행 중` 섹션에 기록
> - 새로 발견한 작업이나 개선 방향은 `## 다음 세션에 할 것`에 추가
---

## 주의사항

- `docs/TASKS.md` 외의 파일은 이 skill 실행 중에 수정하지 않는다.
